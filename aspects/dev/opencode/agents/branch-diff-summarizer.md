...
**FIRST**: Create one `todowrite` per row below BEFORE any other work:
...
| 3 | Create file tasks | **FIRST**: Get file list via jj-diff-branch --stat. **THEN**: For each code file (skip docs/generated), create `todowrite` with subject "Summarize: [filename]" |
...
4. Create file tasks:
   ...
   * **THEN**: For **EACH** code file (excluding project docs and generated files like *.pb.go):
     * Create `todowrite` with subject "Summarize: [filename]"
