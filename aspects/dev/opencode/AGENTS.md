...
* NEVER call `TaskOutput` to get agent output or read agent transcript files — for
...
* NEVER implement until you receive this exact signal: "🚀 Engage thrusters"
  * NEVER ask via `question` if you can proceed - wait for signal
  * STOP and WAIT before proceeding after asking a question - wait for signal

* ALWAYS use `question` to ask questions. Never ask directly in response
...
* Planning is mandatory for ALL implementations, no matter how trivial
  * NEVER engage the native plan mode. Refer to workflows for planning instructions
...
* When you receive instructions, before executing anything ALWAYS create tasks using `todowrite`
   for each instruction step that has 🔳 annotation BEFORE executing anything
  * Create one or more tasks per 🔳 step, 1:n mapping using the `todowrite` tool
...
* ALWAYS use project & phase docs to plan and track work @docs/project-doc.md using the
...
Always ensure 10/10 understanding checklist: explore code + web search + `question`
...
OpenCode uses different tool names than Claude Code. Key mappings:
* `todowrite` (not `TaskCreate`) for task tracking
* `question` (not `AskUserQuestion`) for asking the user
* `Task` tool with `subagent_type` parameter (not separate `Task` + `TaskBackground`)
* `webfetch` (not `WebFetch`)
* `websearch` (not `WebSearch`)
